...
export const Edit = ({formData, image, setFormData,handleClick }) => {
  const fileInput = useRef(null)
  const apiDomain = import.meta.env.VITE_BXDP_DEV_SERVER;

  const link = `url('${formData.logo_url}')`
  ...
  async function handleFileInput (e) {
    // Uploaded file
      ...
      setFormData({ ...formData, logo_url: result.data });
  ...
  return (
    ...
    <div className="flex items-top gap-3 ">
     ...
     <div className=" ml-5 rounded-xl h-[100px] w-[100px]"
          style={{"background":link,"backgroundRepeat":"no-repeat","backgroundPosition":"center","backgroundSize":"contain" }}
      /> 
    </>
  );
};
