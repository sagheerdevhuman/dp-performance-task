...
export const Text = ({ handleSubmit,text,formData,setFormData,admin,rich,title,type,noTitle}) => {
  ...
  if (edit==true){
    return(
      <form  onSubmit={(e) => handleSubmit(e,formData)} >
        <Edit handleClick={handleClick} text={text} formData={formData} setFormData={setFormData} rich={rich} type={type}/>
      </form>
    )
  }
  return (
    <div className="flex gap-3 items-center my-3">
      ...
      <div>
        ...
        {noTitle==false && <h2  className="font-[900] text-[32px] my-[20px] ">{title}</h2>}
        { rich==true &&(
          <div className="break-word" dangerouslySetInnerHTML={markup}/> 
        )}{ rich==false &&(
          <p className="text-[16px] font-[400] mb-[44px] break-word">{text}</p>
        )} 
      </div>   
    </div>
  );
