...
import { Edit } from "./edit/title";

export const Title = ({ handleSubmit, text, formData, setFormData, setText, admin }) => {
  ...
  if (edit==true){
    return(
      <form className="w-[579px]" onSubmit={(e) => handleSubmit(e,formData)} >
        <Edit handleClick={handleClick} text={text} setText={setText} formData={formData} setFormData={setFormData} prevText={text} />
      </form>
    )
  }
  ...
  return (
     <div className="flex items-center gap-3 ">
      ...
      />*/}
      {admin== true &&(
        <svg  className="fill-[#fff] h-[24px] hover:fill-tkh-brand-tangerine-5 transition ease-in-out duration-300 " viewBox="0 0 24 24"  xmlns="http://www.w3.org/2000/svg"  onClick={(e) => { handleClick(e);}}>
      ...
      <h1 className="text-4xl md:text-[72px] leading-tight mb-3 font-bold  break-word">
        {text}
      </h1>
    </div>
  );
