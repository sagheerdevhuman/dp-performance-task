import { RSVP } from "../sharedComponents/RSVP";
import { useNavigate } from "react-router-dom";
import { useState, useEffect } from "react";
import { Confirm } from "./confirm";
import { Edit } from "./edit/rsvp";

export const RSVP_link = ({ handleSubmit,  formData, setFormData, event,admin }) => {
  ...
  if (edit==true){
    return(
      <form className="w-[579px]" onSubmit={(e) => handleSubmit(e,formData)} >
        <Edit handleClick={handleClick} formData={formData} setFormData={setFormData} event={event} />
      </form>
    )
  }
  ...
  return (
     <div className="flex items-center gap-3 ">
      ...
      />*/}
    { admin==true &&(
    	<svg  className="fill-tkh-grayscale-6 h-[24px] hover:fill-tkh-brand-tangerine-5 transition ease-in-out duration-300 " viewBox="0 0 24 24"  xmlns="http://www.w3.org/2000/svg"  onClick={(e) => { handleClick(e);}}>
      ...
      <RSVP id={event.event_id} event={event} />
