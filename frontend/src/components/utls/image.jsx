...
export const Image = ({ handleSubmit,image,setImage,formData,setFormData}) => {
  ...
  if (edit==true){
    return(
      <form className="w-[579px]" onSubmit={(e) => handleSubmit(e,formData)} >
        <Edit handleClick={handleClick} image={image} setImage={setImage} formData={formData} setFormData={setFormData} />
      </form>
    )
  }
  ...
  return (
    <div className="flex h-[90%]  w-full">
      {/*<Confirm 
        ...
        formData={formData}
      />*/}
      <svg  className="fill-tkh-grayscale-6 m-3 h-[24px] hover:fill-tkh-brand-tangerine-5 transition ease-in-out duration-300 " viewBox="0 0 24 24"  xmlns="http://www.w3.org/2000/svg"  onClick={(e) => { handleClick(e);}}>
      ...
      <div className=" rounded-xl h-[100%] z-10 w-[100vw] md:w-full "
      	style={{"background":image_link,"backgroundRepeat":"no-repeat","backgroundPosition":"center","backgroundSize":"cover" }}
      /> 
    </div>
  );
};
