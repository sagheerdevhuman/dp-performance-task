...
export const Filter = ({ data, setData, type, originalData}) => {
  const [text, setText] = useState();
  const id = `${type}_id`
  const placeholder = `by ${type}`

  const setOptions = (originalData) => {
    if(originalData){
      return originalData.map((item) => ({
        value: item[id],
        label: type === "user" ? `${item.first_name} ${item.last_name}` : item.name,
      }));
    }
  ...
  const options = setOptions(originalData);

  const handleChange = (value) => {
      setText(value);
     
      if (value && value.value) {
        console.log(`Filter applied for ${type}:`, value.value);
        setData(originalData.filter(x=> x[id] == value.value ));
      } else {
        // When filter is cleared, show all data
        console.log(`Filter cleared for ${type}`);
