...
export function CommunityEvents({data}) {
  ...
  const handleEventClick = (event) => {
    if (event.type==="event") {
      ...
      navigate(`/program_details/${event.id}`)
    }
  };
...
console.log("index",index === 1  ? "bg-[#FF7000]" : "bg-[#fff]")
      ...
      >
        ...
        <div className="mt-auto">
          ...
          {event.type==="program" && (
            ...
            <p className="text-sm mb-4">
            {ft.getFormattedTime(event.start_time.replace(/(:\d{2}| [AP]M)$/, ""))}{" "}-{" "}{ft.getFormattedTime(event.end_time.replace(/(:\d{2}| [AP]M)$/, ""))}
            </p>
            {/* <p className="text-sm mb-4">
            {event.week_days[0]} {event.week_days[1]} {event.week_days[2]} {event.week_days[3]} {event.week_days[4]} {event.week_days[5]} {event.week_days[6]}
            </p> */}
            </>
          )}
          
          {event.organization?.logo_url && (
            <img src={event.organization?.logo_url} 
  ...
  return (
    <div className="relative px-6 py-12 bg-[url('https://d1yh21d3dzz97r.cloudfront.net/slider.png')]">
      <h1 className="text-4xl font-bold text-center text-white mb-12">Event and Program Recommendations</h1>
          ...
          {groupedEvents.map((group, groupIndex) => (
            <div key={groupIndex} className="grid grid-cols-3 gap-6 px-6 carousel-container">
              {group.map((event, index) => (
                <CarouselItem 
                  index={index}
                  key={event?.id || `empty-${groupIndex}-${index}`} 
